<?php
/*
Plugin Name: Redapple Calendar
Description: This plugin allows you to display a calendar of all your events and appointments as a page on your site. Built for Redapple Theme, but can be used anywhere
Author: Melissa Cabral
Author URI: http://www.melissacabral.com
Version: 1

Based on Calendar by Kieran O'Shea. HTML modified to be more standards based
...
function calendar_menu() 
{
  ...
  // Add the admin panel pages for Calendar. Use permissions pulled from above
   if (function_exists('add_menu_page')) 
     {
       add_menu_page('Calendar', 'Calendar', $allowed_group, 'calendar', 'edit_calendar');
     }
   if (function_exists('add_submenu_page')) 
     {
       $calendar_manage_menu = add_submenu_page('calendar', 'calendar', 'Manage Calendar', $allowed_group, 'calendar', 'edit_calendar');
      
       // Note only admin can change calendar options
       add_submenu_page('calendar', 'Manage Categories','Manage Categories', 'manage_options', 'calendar-categories', 'manage_categories');
       add_submenu_page('calendar', 'Calendar Config', 'Calendar Options', 'manage_options', 'calendar-config', 'edit_calendar_config');
     }
}
 add_action( 'admin_enqueue_scripts', 'calendar_add_javascript' );
// Function to add the javascript to the admin header
function calendar_add_javascript( $hook )
{ 
 
  if('toplevel_page_calendar' != $hook)
    return;

  $js_path = plugins_url( 'javascript.js', __FILE__ );
  wp_register_script( 'mc_calendar_js', $js_path  );
  wp_enqueue_script( 'mc_calendar_js');
  $css_path = plugins_url( 'calendar.css', __FILE__ );
  wp_enqueue_style( 'mc_calendar_css', $css_path );
}
...
?>
       	<table class="widefat page fixed" width="100%" cellpadding="3" cellspacing="3">
		        <thead>
			    <tr>
				<th class="manage-column" scope="col"><?php echo 'ID' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Title' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Start Date' ?></th>
				<th class="manage-column" scope="col"><?php echo 'End Date' ?></th>
		                <th class="manage-column" scope="col"><?php echo 'Time' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Recurs' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Repeats' ?></th>
		                <th class="manage-column" scope="col"><?php echo 'Author' ?></th>
		                <th class="manage-column" scope="col"><?php echo 'Category' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Edit' ?></th>
				<th class="manage-column" scope="col"><?php echo 'Delete' ?></th>
			    </tr>
		        </thead>
<?php
		$class = '';
		foreach ( $events as $event )
		{
			...
			<tr class="<?php echo $class; ?>">
				...
				<td><?php if ($event->event_time == '00:00:00') { echo 'N/A'; } else { echo stripslashes($event->event_time); } ?></td>
				<td>
				<?php 
					...
					else if ($event->event_recur == 'M') { echo 'Monthly (date)'; }
			                else if ($event->event_recur == 'U') { echo 'Monthly (day)'; }
				...
				<?php
					...
					else if ($event->event_repeats > 0) { echo stripslashes($event->event_repeats).' '.'Times'; }					
				?>
				</td>
				...
				<?php unset($this_cat); ?>
				<td><a href="<?php echo bloginfo('wpurl') ?>/wp-admin/admin.php?page=calendar&amp;action=edit&amp;event_id=<?php echo stripslashes($event->event_id);?>" class='edit'><?php echo 'Edit'; ?></a></td>
				<td><a href="<?php echo bloginfo('wpurl') ?>/wp-admin/admin.php?page=calendar&amp;action=delete&amp;event_id=<?php echo stripslashes($event->event_id);?>" class="delete" onclick="return confirm('<?php echo 'Are you sure you want to delete this event?'; ?>')"><?php echo 'Delete'; ?></a></td>
			</tr>
			<?php
		}
		?>
	...
	{
		?>
		<p><?php echo "There are no events in the database!"	?></p>
		<?php	
	}
}
	...
	<form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo bloginfo('wpurl'); ?>/wp-admin/admin.php?page=calendar">
		...
		<div id="linkadvanceddiv" class="postbox">
			<div style="float: left; width: 98%; clear: both;" class="inside">
                                ...
                                <tr>				
				<td><legend><?php echo 'Event Title'; ?></legend></td>
				<td><input type="text" name="event_title" class="input" size="40" maxlength="30"
					value="<?php if ( !empty($data) ) echo htmlspecialchars(stripslashes($data->event_title)); ?>" /></td>
                                ...
                                <tr>
				<td style="vertical-align:top;"><legend><?php echo 'Event Description'; ?></legend></td>
				<td><textarea name="event_desc" class="input" rows="5" cols="50"><?php if ( !empty($data) ) echo htmlspecialchars(stripslashes($data->event_desc)); ?></textarea></td>
                                ...
                                <tr>
				<td><legend><?php echo 'Event Category'; ?></legend></td>
...
';
                                ...
                                <tr>
				<td><legend><?php echo 'Event Link (Optional)'; ?></legend></td>
                                ...
                                <tr>
				<td><legend><?php echo 'Start Date'; ?></legend></td>
                                <td>    
						...
						var cal_1 = new Calendar({
							...
							onSelect: function unifydates(element) {
							  document.forms['quoteform'].event_end.value = document.forms['quoteform'].event_begin.value;
                                ...
                                <tr>
				<td><legend><?php echo 'End Date'; ?></legend></td>
                                <td>    
					...
					<script type="text/javascript">
						var cal_2 = new Calendar({
							...
							minDate: new Date(parseInt(document.forms['quoteform'].event_begin.value.split('-')[0]),parseInt(document.forms['quoteform'].event_begin.value.split('-')[1]-1),parseInt(document.forms['quoteform'].event_begin.value.split('-')[2]))
                                ...
                                <tr>
				<td><legend><?php echo 'Time (hh:mm)'; ?></legend></td>
				<td>	<input type="time" name="event_time" class="input" size=12
					value="<?php 
					if ( !empty($data) ) 
					{
					...
					?>" /> <?php echo 'Optional, set blank if not required.'; ?> <?php echo 'Current time difference from GMT is '; echo get_option('gmt_offset'); echo ' hour(s)'; ?>
				</td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Recurring Events'; ?></legend></td>
				<td>	<?php
					if (isset($data)) {
					...
					?>
					  <?php echo 'Repeats for'; ?> 
					<input type="text" name="event_repeats" class="input" size="1" value="<?php echo $repeats; ?>" /> 
					<select name="event_recur" class="input">
						<option class="input" <?php echo $selected_s; ?> value="S"><?php echo 'None' ?></option>
						<option class="input" <?php echo $selected_w; ?> value="W"><?php echo 'Weeks' ?></option>
						<option class="input" <?php echo $selected_m; ?> value="M"><?php echo 'Months (date)' ?></option>
						<option class="input" <?php echo $selected_u; ?> value="U"><?php echo 'Months (day)' ?></option>
						<option class="input" <?php echo $selected_y; ?> value="Y"><?php echo 'Years' ?></option>
					</select><br />
					<?php echo 'Entering 0 means forever. Where the recurrance interval is left at none, the event will not reoccur.'; ?>
				</td>
                                </tr>
                                </table>
			</div>
			<div style="clear:both; height:1px;">&nbsp;</div>
                ...
                <input type="submit" name="save" class="button bold" value="<?php echo 'Save'; ?> &raquo;" />
	</form>
	<?php
}
...
if ( $action == 'add' )
{
	...
	if ((preg_match($date_format_one,$begin) || preg_match($date_format_two,$begin)) && (preg_match($date_format_one,$end) || preg_match($date_format_two,$end)))
	  {
	    ...
	    $end_d = $end_split[2];
		 ...
		 {
		   ?>
		   <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'Your event end date must be either after or the same as your event begin date'; ?></p></div>
		   <?php
		 }
	     } 
	    else
	      {
		?>
                <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'Your date formatting is correct but one or more of your dates is invalid. Check for number of days in month and leap year related errors.'; ?></p></div>
                <?php
	      }
	  }
	else
	  {
	    ?>
            <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'Both start and end dates must be entered and be in the format YYYY-MM-DD'; ?></p></div>
            <?php
	  }
	...
	$time_format_two = '/^([2][0-3]):([0-5][0-9])$/';
		...
		$time_to_use = $time;
          ...
          {
            ?>
            <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'The time field must either be blank or be entered in the format hh:mm'; ?></p></div>
            <?php
	  }
	...
	if (preg_match('/^(http)(s?)(:)\/\//',$linky) || $linky == '')
	  ...
	  {
              ?>
              <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'The URL entered must either be prefixed with http:// or be completely blank'; ?></p></div>
              <?php
	  }
	...
	if (preg_match('/^.{1,30}$/',$title))
	  ...
	  {
              ?>
              <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'The event title must be between 1 and 30 characters in length'; ?></p></div>
              <?php
	  }
	...
	if (($repeats == 0 && $recur == 'S') || (($repeats >= 0) && ($recur == 'W' || $recur == 'M' || $recur == 'Y' || $recur == 'U')))
	  ...
	  {
              ?>
              <div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'The repetition value must be 0 unless a type of recurrance is selected in which case the repetition value must be 0 or higher'; ?></p></div>
              <?php
	  }
	if (isset($start_date_ok) && isset($end_date_ok) && isset($time_ok) && isset($url_ok) && isset($title_ok) && isset($recurring_ok))
	  {
	    ...
	    if ( empty($result) || empty($result[0]->event_id) )
	      {
                ?>
		<div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'An event with the details you submitted could not be found in the database. This may indicate a problem with your database or the way in which it is configured.'; ?></p></div>
		<?php
	      }
	    else
	      {
		?>
		<div class="updated"><p><?php echo 'Event added. It will now show in your calendar.'; ?></p></div>
		<?php
	      }
	  }
	else
...
elseif ( $action == 'edit_save' )
{
	...
	{
		?>
		<div class="error"><p><b><?php echo 'Failure'; ?>:</b> <?php echo "You can't update an event if you haven't submitted an event id"; ?></p></div>
		<?php		
	}
	else
	{
	  ...
	  if (isset($start_date_ok) && isset($end_date_ok) && isset($time_ok) && isset($url_ok) && isset($title_ok) && isset($recurring_ok))
		...
		{
			?>
			<div class="error"><p><b><?php echo 'Failure'; ?>:</b> <?php echo 'The database failed to return data to indicate the event has been updated sucessfully. This may indicate a problem with your database or the way in which it is configured.'; ?></p></div>
			<?php
		}
		else
		{
			?>
			<div class="updated"><p><?php echo 'Event updated successfully'; ?></p></div>
			<?php
		}
	    }
          else
...
elseif ( $action == 'delete' )
{
	...
	{
		?>
		<div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo "You can't delete an event if you haven't submitted an event id"; ?></p></div>
		<?php			
	}
	else
	{
		...
		{
			?>
			<div class="updated"><p><?php echo 'Event deleted successfully'; ?></p></div>
			<?php
		}
		else
		{
			?>
			<div class="error"><p><b><?php echo 'Error'; ?>:</b> <?php echo 'Despite issuing a request to delete, the event still remains in the database. Please investigate.'; ?></p></div>
			<?php
...
<div class="wrap">
	...
	{
		?>
		<h2><?php echo 'Edit Event'; ?></h2>
		<?php
		if ( empty($event_id) )
		{
			echo "<div class=\"error\"><p>"."You must provide an event id in order to edit it"."</p></div>";
		}
		else
		{
	...
	{
		?>
		<h2><?php echo 'Add Event'; ?></h2>
		<?php wp_events_edit_form(); ?>
	
		<h2><?php echo 'Manage Events'; ?></h2>
		<?php
...
function edit_calendar_config()
{
  ...
  <div class="wrap">
  <h2><?php echo 'Calendar Options'; ?></h2>
  <form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo bloginfo('wpurl'); ?>/wp-admin/admin.php?page=calendar-config">
                <div id="linkadvanceddiv" class="postbox">
                        <div style="float: left; width: 98%; clear: both;" class="inside">
                                <table cellpadding="5" cellspacing="5">
				<tr>
                                <td><legend><?php echo 'Choose the lowest user group that may manage events'; ?></legend></td>
				<td>        <select name="permissions">
				            <option value="subscriber"<?php echo $subscriber_selected ?>><?php echo 'Subscriber'?></option>
				            <option value="contributor" <?php echo $contributor_selected ?>><?php echo 'Contributor'?></option>
				            <option value="author" <?php echo $author_selected ?>><?php echo 'Author'?></option>
				            <option value="editor" <?php echo $editor_selected ?>><?php echo 'Editor'?></option>
				            <option value="admin" <?php echo $admin_selected ?>><?php echo 'Administrator'?></option>
				        </select>
                                </td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Do you want to display the author name on events?'; ?></legend></td>
                                <td>    <select name="display_author">
                                        <option value="on" <?php echo $yes_disp_author ?>><?php echo 'Yes' ?></option>
                                        <option value="off" <?php echo $no_disp_author ?>><?php echo 'No' ?></option>
                                    </select>
                                </td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Display a jumpbox for changing month and year quickly?'; ?></legend></td>
                                <td>    <select name="display_jump">
                                         <option value="on" <?php echo $yes_disp_jump ?>><?php echo 'Yes' ?></option>
                                         <option value="off" <?php echo $no_disp_jump ?>><?php echo 'No' ?></option>
                                    </select>
                                </td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Display todays events?'; ?></legend></td>
                                <td>    <select name="display_todays">
						<option value="on" <?php echo $yes_disp_todays ?>><?php echo 'Yes' ?></option>
						<option value="off" <?php echo $no_disp_todays ?>><?php echo 'No' ?></option>
                                    </select>
                                </td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Display upcoming events?'; ?></legend></td>
                                <td>    <select name="display_upcoming">
						<option value="on" <?php echo $yes_disp_upcoming ?>><?php echo 'Yes' ?></option>
						<option value="off" <?php echo $no_disp_upcoming ?>><?php echo 'No' ?></option>
                                    </select>
				    <?php echo 'for'; ?> <input type="text" name="display_upcoming_days" value="<?php echo $upcoming_days ?>" size="1" maxlength="2" /> <?php echo 'days into the future'; ?>
                                </td>
                                </tr>
                                <tr>
				<td><legend><?php echo 'Enable event categories?'; ?></legend></td>
                                <td>    <select name="enable_categories">
				                <option value="on" <?php echo $yes_enable_categories ?>><?php echo 'Yes' ?></option>
						<option value="off" <?php echo $no_enable_categories ?>><?php echo 'No' ?></option>
                                    </select>
                                </td>
                                </tr>
                                <tr>
				<td style="vertical-align:top;"><legend><?php echo 'Configure the stylesheet for Calendar'; ?></legend></td>
				<td><textarea name="style" rows="10" cols="60" tabindex="2"><?php echo $calendar_style; ?></textarea><br />
                                <input type="checkbox" name="reset_styles" /> <?php echo 'Tick this box if you wish to reset the Calendar style to default'; ?></td>
                                </tr>
                                </table>
			</div>
                        <div style="clear:both; height:1px;">&nbsp;</div>
  ...
  if (isset($_POST['mode']) && $_POST['mode'] == 'add')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><b>".'Category added successfully'."</b></p></div>";
    }
  else if (isset($_GET['mode']) && isset($_GET['category_id']) && $_GET['mode'] == 'delete')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><b>".'Category deleted successfully'."</b></p></div>";
    }
...
<div class="wrap">
   <h2><?php echo 'Edit Category'; ?></h2>
    <form name="catform" id="catform" class="wrap" method="post" action="<?php echo bloginfo('wpurl'); ?>/wp-admin/admin.php?page=calendar-categories">
				...
				<table cellpadding="5" cellspacing="5">
                                <tr>
				<td><legend><?php echo 'Category Name'; ?>:</legend></td>
                                ...
                                <tr>
				<td><legend><?php echo 'Category Colour (Hex format)'; ?>:</legend></td>
  ...
  else if (isset($_POST['mode']) && isset($_POST['category_id']) && isset($_POST['category_name']) && isset($_POST['category_colour']) && $_POST['mode'] == 'edit')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><b>".'Category edited successfully'."</b></p></div>";
    }
...
?>
  ...
  <div class="wrap">
    ...
    <h2><?php echo 'Manage Categories'; ?></h2>
<?php
 ...
 if ( !empty($categories) )
   {
     ...
     <table class="widefat page fixed" width="50%" cellpadding="3" cellspacing="3">
       ...
       <tr>
         <th class="manage-column" scope="col"><?php echo 'ID' ?></th>
	 <th class="manage-column" scope="col"><?php echo 'Category Name' ?></th>
	 <th class="manage-column" scope="col"><?php echo 'Category Colour' ?></th>
	 <th class="manage-column" scope="col"><?php echo 'Edit' ?></th>
	 <th class="manage-column" scope="col"><?php echo 'Delete' ?></th>
       </tr>
       </thead>
	   ...
	   $class = ($class == 'alternate') ? '' : 'alternate';
	     ...
	     <td><a href="<?php echo bloginfo('wpurl')  ?>/wp-admin/admin.php?page=calendar-categories&amp;mode=edit&amp;category_id=<?php echo stripslashes($category->category_id);?>" class='edit'><?php echo 'Edit'; ?></a></td>
	     <?php
		 ...
		 echo '<td>'.'N/A'.'</td>';
	       ...
	       {
               ?>
               <td><a href="<?php echo bloginfo('wpurl') ?>/wp-admin/admin.php?page=calendar-categories&amp;mode=delete&amp;category_id=<?php echo stripslashes($category->category_id);?>" class="delete" onclick="return confirm('<?php echo 'Are you sure you want to delete this category?'; ?>')"><?php echo 'Delete'; ?></a></td>
               <?php
	       }
                ?>
              </tr>
   ...
   {
     echo '<p>'.'There are no categories in the database - something has gone wrong!'.'</p>';
   }
...
function next_link($cur_year,$cur_month,$minical = false)
{
  ...
  if ($cur_month == 12)
    {
      if ($minical) { $rlink = ''; } else { $rlink = 'Next'; }
...
function prev_link($cur_year,$cur_month,$minical = false)
{
  ...
  if ($cur_month == 1)
    {
      if ($minical) { $llink = ''; } else { $llink = 'Prev'; }
  ...
  if ($display == 'true')
      ...
      while ($day_count < $future_days+1)
	{
	  ...
	  foreach($events as $event)
		...
		$time_string = ' '.'at'.' '.date(get_option('time_format'), strtotime(stripslashes($event->event_time)));
	      }
...
function widget_init_events_calendar() { 
  ...
  function widget_events_calendar($args) {
    ...
    $widget_title = empty($the_title) ? 'Calendar' : $the_title;
  ...
  function widget_events_calendar_control() {
    ...
    ?>
    <p>
       <label for="events_calendar_widget_title"><?php echo 'Title'; ?>:<br />
       ...
       <label for="events_calendar_widget_cats"><?php echo 'Comma separated category id list'; ?>:<br />
  ...
  wp_register_sidebar_widget('events_calendar','Event Calendar','widget_events_calendar',array('description'=>'A calendar of your events'));
...
function widget_init_calendar_today() {
  ...
  function widget_calendar_today($args) {
    ...
    $widget_title = empty($the_title) ? 'Today\'s Events' : $the_title;
  ...
  function widget_calendar_today_control() {
    ...
    ?>
    <p>
       <label for="calendar_today_widget_title"><?php echo 'Title'; ?>:<br />
       ...
       <label for="calendar_today_widget_cats"><?php echo 'Comma separated category id list'; ?>:<br />
  ...
  wp_register_sidebar_widget('todays_events_calendar','Today\'s Events','widget_calendar_today',array('description'=>'A list of your events today'));
...
function widget_init_calendar_upcoming() {
  ...
  function widget_calendar_upcoming($args) {
    ...
    $widget_title = empty($the_title) ? 'Upcoming Events' : $the_title;
  ...
  function widget_calendar_upcoming_control() {
    ...
    ?>
    <p>
       <label for="calendar_upcoming_widget_title"><?php echo 'Title'; ?>:<br />
       ...
       <label for="calendar_upcoming_widget_cats"><?php echo 'Comma separated category id list'; ?>:<br />
  ...
  wp_register_sidebar_widget('upcoming_events_calendar','Upcoming Events','widget_calendar_upcoming',array('description'=>'A list of your upcoming events'));
...
function draw_event($event)
{
  ...
  //$header_details =  '<h3 class="event-title" '.$style.'>'.stripslashes($event->event_title).'</h3>';
  if ($event->event_time != "00:00:00")
    {
      $header_details .= '<b>'.'Time'.':</b> ' . date(get_option('time_format'), strtotime(stripslashes($event->event_time))) . '<br />';
    }
  if ($display_author == 'true')
    {
      $e = get_userdata(stripslashes($event->event_author));
      $header_details .= '<b>Posted by</b> '.$e->display_name.'<br />';
    }
  ...
  $details = '<div class="calnk"'.$style.'><a href="'.$linky.'" ><h3>' . stripslashes($event->event_title) . '</h3><div class="cal-details">' . $header_details . '' . stripslashes($event->event_desc) . '</div></a></div>';
...
';
	...
	$calendar_body .= '
            ...
            <option value="jan"'.calendar_month_comparison('jan').'>'.'January'.'</option>
            <option value="feb"'.calendar_month_comparison('feb').'>'.'February'.'</option>
            <option value="mar"'.calendar_month_comparison('mar').'>'.'March'.'</option>
            <option value="apr"'.calendar_month_comparison('apr').'>'.'April'.'</option>
            <option value="may"'.calendar_month_comparison('may').'>'.'May'.'</option>
            <option value="jun"'.calendar_month_comparison('jun').'>'.'June'.'</option>
            <option value="jul"'.calendar_month_comparison('jul').'>'.'July'.'</option> 
            <option value="aug"'.calendar_month_comparison('aug').'>'.'August'.'</option> 
            <option value="sept"'.calendar_month_comparison('sept').'>'.'September'.'</option> 
            <option value="oct"'.calendar_month_comparison('oct').'>'.'October'.'</option> 
            <option value="nov"'.calendar_month_comparison('nov').'>'.'November'.'</option> 
            <option value="dec"'.calendar_month_comparison('dec').'>'.'December'.'</option> 
            </select>
		...
		if (get_option('start_of_week') == 0)
		  {
		    ...
		    $calendar_body .= '        <td class="'.(date("Ymd", mktime (0,0,0,$c_month,$i,$c_year))==date("Ymd",ctwo())?'current-day':'day-with-date').$no_events_class.'"><span class="number'.($ii<7&&$ii>1?'':' weekend"').'">'.$i++.'</span><div class="event">' . draw_events($grabbed_events) . '</div></td>
';
		  }
		else
		  {
...
';
	...
	$cat_details = $wpdb->get_results($sql);
        $calendar_body .= '<div class="cat-key"><h3>'.'Category Key'.'</h3><ul>';
        foreach($cat_details as $cat_detail)
	  {
	    $calendar_body .= '<li><div style="width:20px; height:20px;background-color:'.$cat_detail->category_colour.';" class="cat-key-cell"></div>'.$cat_detail->category_name.'</li>';
	  }
        $calendar_body .= '</ul></div>';
      }
...
function minical_draw_events($events,$day_of_week = '')
{
  ...
  if (count($events)) {
    // Setup the wrapper
    $output = '<div class="calnk"><a href="#">'.$day_of_week.'<span class="flyout">';
    // Now process the events
    foreach($events as $event)
      {
	if ($event->event_time == '00:00:00') { $the_time = ''; } else { $the_time = 'at '.date(get_option('time_format'), strtotime(stripslashes($event->event_time))); } 
	$output .= '<h3 class="event-title">'.$event->event_title.'</h3> <time class="event-date">'.$the_time.'</time>';
      }
    // The tail
    $output .= '</span></a></div>';
  } else {
...
function minical($cat_list = '') {
  ...
  if (get_option('start_of_week') == 0)
    {
      $name_days = array(1=>'Su','Mo','Tu','We','Th','Fr','Sa');
    }
  // Choose Monday if anything other than Sunday is set                                                                                                                           
  else
    {
      $name_days = array(1=>'Mo','Tu','We','Th','Fr','Sa','Su');
    }
  ...
  // Start the table and add the header and naviagtion                                                                                                                            
  $calendar_body = '';
  $calendar_body .= '<div><table cellspacing="0" cellpadding="0" class="calendar-table">
';
...
';
    ...
    $calendar_body .= '<div class="credit-link" style="visibility:visible !important;display:block !important;"><p>Calendar by <a href="http://www.melissacabral.com">MC</a></p></div>
';
